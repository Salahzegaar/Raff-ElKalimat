...
const BookCard: React.FC<BookCardProps> = ({ book, onSelect, size = 'normal', style, className, isFavorite, onToggleFavorite }) => {
  ...
  return (
    ...
    >
      <div className={`relative ${imageContainerHeight} bg-gray-200 dark:bg-gray-800 flex items-center justify-center`}>
        ...
        {coverUrl ? (
          <img src={coverUrl} alt={`Cover for ${book.title}`} className="h-full w-full object-cover" />
        ) : (
                ...
                {book.subject && book.subject.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        {book.subject.slice(0, 3).map((s, i) => (
                            <span key={i} className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full truncate" title={s}>
                                {s}
                            </span>
                        ))}
                    </div>
